...
const Counter = () => {
  ...
  const [refreshOrdersKey, setRefreshOrdersKey] = useState(0); // Key to force re-render/refresh of Todayorder
  ...
  const calculateRowTotal = (sell_price, quantity) => {
    ...
    if (e.key === "Enter") {
      ...
      if (item) {
        ...
        if (currentDate > expiryDate) {
          setError(`Item ${item.name} has expired (Exp: ${item.expiry}). Cannot add.`);
          setBarcodeInput("");
        } else {
          setItems((prevItems) => [
            ...prevItems,
  ...
  useEffect(() => {
    const total = items.reduce(
      (sum, item) => sum + calculateRowTotal(item.sell_price, item.quantity),
      0
    );
    setBillTotal(total);
  ...
  const handleSave = async () => {
    ...
    const postData = {
      ...
      items: items.map((item) => ({
        ...
        totalAmount: calculateRowTotal(item.sell_price, item.quantity),
        totalcostamount: calculateRowTotal(item.cost_price, item.quantity),
      })),
    };
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/pharmacy/invoice/add`,
        postData
      );
      ...
      if (response.status === 201) {
        ...
        setRefreshOrdersKey(prev => prev + 1); // Refresh Today's Orders list
        setError("");
      } else {
        setError(`Failed to save: ${response.data ? response.data.message : response.statusText}`);
      }
    } catch (error) {
      console.error("Error saving invoice data:", error);
      const msg = error.response?.data?.message || error.response?.data || error.message;
      setError(`Error: ${typeof msg === 'object' ? JSON.stringify(msg) : msg}`);
    }
  };
  ...
  const handleClear = () => {
    ...
    setError("");
    // Keep success message visible for a moment if desired, but user asked to clear.
    // If handleClear is called manually 'Clear' button, we might want to clear success too.
    // If called from handleSave, maybe we want to keep it? 
    // Let's clear everything as requested "clear values placed in text fields".
    setBillTotal(0);
    setAmountReceived("");
    setBalancePayable("");
    setCashPayable(0);
  ...
  useEffect(() => {
    ...
    if (savedItems) {
      ...
      const initialTotal = calculateGrandTotal(parsedItems.items);
  ...
  useEffect(() => {
    ...
    const updatedTotal = calculateGrandTotal(items);
  ...
  const downloadInvoice = async () => {
    ...
    const sendEmail = async () => {
      ...
      const url = `${import.meta.env.VITE_API_BASE_URL}/pharmacy/send-invoice-email`;
      console.log(email);
  ...
  return (
    <div className="counter_container">
      {error && <p className="error" style={{ color: 'red', textAlign: 'center', padding: '10px' }}>{error}</p>}
      {success && <p className="success" style={{ color: 'green', textAlign: 'center', padding: '10px' }}>{success}</p>}
                ...
                <tbody>
                  {items.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="right">
                        {calculateRowTotal(item.sell_price, item.quantity)}
                      </td>
                    </tr>
                  ))}
                  <tr>
      ...
      <div className="bottom-content">
        <div className="left" style={{ height: "45vh", overflowY: "scroll" }}>
          <Todayorder trigger={refreshOrdersKey} />
        </div>
        <div className="right">
          <div
            className="content"
