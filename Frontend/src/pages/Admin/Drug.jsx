...
const Drug = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        editMode
          ? `${import.meta.env.VITE_API_BASE_URL}/pharmacy/drug/update/${editId}`
          : `${import.meta.env.VITE_API_BASE_URL}/pharmacy/drug/add`,
        {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/pharmacy/drug/del/${id}`,
        {
          method: "DELETE",
        }
      );
              ...
              {data.map((drug, id) => {
                return (
                  <tr key={id}>
                    ...
                    <td>{drug.quantity}</td>
                    <td>
                      {dayjs(drug.createdAt).format("DD-MM-YYYY hh:mm:ss")}
                    </td>
                    <td className="buttonss">
                      <button onClick={() => handleEdit(drug)}>Edit</button>
                      <button onClick={() => handleDelete(drug.barcode)}>
                        Delete
                      </button>
                    </td>
                  </tr>
