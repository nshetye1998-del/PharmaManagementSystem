...
const HistorySale = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('https://api-5e1h.onrender.com/pharmacy/historysale/all');
            if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (isEditing) {
                response = await fetch(`https://api-5e1h.onrender.com/pharmacy/historysale/update/${currentId}`, {
                    method: 'PUT',
                    headers: {
                ...
                response = await fetch("https://api-5e1h.onrender.com/pharmacy/historysale/add", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            if (!response.ok) {
                throw new Error(`Failed to ${isEditing ? 'update' : 'add'} history sale: ${response.status} - ${response.statusText}`);
            }

            if (isEditing) {
                setData(data.map((item) => (item._id === currentId ? responseData : item)));
            } else {
            ...
            console.log(`History sale ${isEditing ? 'updated' : 'added'} successfully:`, responseData);
            alert(`History sale ${isEditing ? 'updated' : 'added'} successfully!`);
        } catch (error) {
            console.error(`Failed to ${isEditing ? 'update' : 'add'} history sale:`, error.message);
            setError(`Failed to ${isEditing ? 'update' : 'add'} history sale. Please try again.`);
            alert(`Failed to ${isEditing ? 'update' : 'add'} history sale. Please try again.`);
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const response = await fetch(`https://api-5e1h.onrender.com/pharmacy/historysale/del/${id}`, {
                method: 'DELETE',
            });

            if (!response.ok) {
                throw new Error(`succesfully delete history sale: ${response.status} - ${response.statusText}`);
            }
        ...
        <section className="purchase-main">
            <div className="purchase-container">
                ...
                <form onSubmit={handleSubmit} className="purchase-form">
                    ...
                    <input type="text" value={barcode} onChange={(e) => setBarcode(e.target.value)} placeholder='Barcode' required />
                    <input type="text" value={dose} onChange={(e) => setDose(e.target.value)} placeholder='Dose' required />
                    ...
                    {isEditing && <button type="button" onClick={clearFields}>Cancel</button>}
                </form>
            </div>
                    ...
                    <tbody>
                        {data.map((sale, id) => (
                            <tr key={id}>
                                ...
                                <td>{sale.price}</td>
                                <td>{sale.amount}</td>
                                <td>{sale.createdAt.slice(0, 10)}</td> {/* Extracting date from createdAt field */}
                                <td>{sale.createdAt.slice(11, 19)}</td> {/* Extracting time from createdAt field */}
                                <td>{sale.name}</td>
                                <td>{sale.quantity}</td>
