...
import PurchaseReport from '../components/Purchasereport'; // Import the PurchaseReport component
import Expired from '../components/Expired'; // Import the Expired component
  ...
  const fetchTotalCustomers = async () => {
    try {
      const response = await fetch(
        "https://api-5e1h.onrender.com/pharmacy/users/all"
      );
      if (!response.ok) {
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-menus">
        <button className="menu-button" onClick={() => setActiveComponent('SaleReport')}>
          Sell Report
        </button>
        <button className="menu-button" onClick={() => setActiveComponent('PurchaseReport')}>
          Purchase Report
        </button>
        <button className="menu-button" onClick={() => setActiveComponent('Expired')}>
          Expired
        </button>
        <button className="menu-button" onClick={() => setActiveComponent('Store')}>
          Stores
        </button>
      </div>
                    ...
                    {displayData.map((data, index) => (
                      <tr key={index}>
                        {Object.keys(data)
                          ...
                          .map((key, idx) => (
                            <td key={idx}>
                              {key === "phone" ? data[key].toString() : data[key]}
                            </td>
                          ))}
                      </tr>
                    ))}
