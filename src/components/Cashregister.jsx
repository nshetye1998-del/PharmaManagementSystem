...
const Cashregister = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (valid) {
      try {
        const response = await fetch('https://api-5e1h.onrender.com/pharmacy/users/register', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ name: name, Email: email, dob: dob, phone: phone, address: address, salary: parseInt(salary), password: password }),
        });
        ...
        if (response.ok) {
          ...
          let errorMessage = 'Registration failed due to a server error. Please try again later.';
